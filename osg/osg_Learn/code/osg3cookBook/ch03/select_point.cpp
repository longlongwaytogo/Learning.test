...
class SelectPointPicker :public osgComm::PickHandler
{
	public:
	SelectPointPicker(osg::Camera* camera):_camera(camera),_selector(0){}
	...
	virtual void doUserOperations(const osgUtil::LineSegmentIntersector::Intersection& in)
	{
        ...
        if(!geometry || !_selector || geometry == _selector) return;
        {
			...
			if(!vertices || !selVertics) return;
			 ...
			 // Compute the world-intersection point and matrix 
			 osg::Vec3 point = in.getWorldIntersectPoint();
			 osg::Matrix matrix = osg::computeLocalToWorld(in.nodePath);
...
void main()
{
	...
	osg::ref_ptr<SelectPointPicker> picker = new SelectPointPicker(viewer.getCamera());
    viewer.addEventHandler(picker);
	root->addChild(picker->createPointSelector());
