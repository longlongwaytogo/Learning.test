...
{
    Object^ dumpyViewerObject = gcnew Object;
    while(!g_viewer->done())
    {

        Monitor::Enter(dumpyViewerObject);
        g_viewer->frame();
        Monitor::Exit(dumpyViewerObject);
    }
...
void cliTest::osgWindow::initializeOSG()
{
    ...
    g_viewer->setSceneData(osgDB::readNodeFile("cow.osg"));

    System::Threading::Thread^ threadObject = gcnew System::Threading::Thread(gcnew System::Threading::ThreadStart(this,&cliTest::osgWindow::runThread));

    //threadObject->p = Threading::ThreadPriority::BelowNormal;
    threadObject->Start();
