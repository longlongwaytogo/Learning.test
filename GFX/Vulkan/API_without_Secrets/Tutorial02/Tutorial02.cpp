    ...
    if( vkEnumerateDeviceExtensionProperties( physical_device, nullptr, &extensions_count, &available_extensions[0] ) != VK_SUCCESS ) {
      ...
      return false;
		...
		if( ( graphics_queue_family_index == UINT32_MAX ) || (present_queue_family_index == UINT32_MAX) ) {
			std::cout<< "Could not find queue family with required properties on physcial device " << physical_device <<" !" << std::endl;
...
#include "ListOfFunctions.inl"
	...
	uint32_t Tutorial02::GetSwapChainNumImages(VkSurfaceCapabilitiesKHR & surface_capabilities) {
		...
		// If application wants to use more iamges at the same time it must ask for more images 
	...
	VkSurfaceFormatKHR Tutorial02::GetSwapChainFormat(std::vector<VkSurfaceFormatKHR> &surface_formats ) {
		...
		// it means tha there are no preferred surface formats and any can be chosen 
	...
	VkExtent2D Tutorial02::GetSwapChainExtent(VkSurfaceCapabilitiesKHR & surface_capabilities ) {
		...
		// specail value of surface exten is width == height  = -1;
		// If this is  we define  the size by ourselves but it must fit within defined confindes
 ...
 bool Tutorial02::CreateCommandBuffers() {
  ...
  bool Tutorial02::Draw() {
	  ...
	  VkSubmitInfo submit_info = {
		  ...
		  &wait_dst_stage_mask,
		  1,
		  &Vulkan.PresentQueueCmdBuffers[image_index],
		  1,
